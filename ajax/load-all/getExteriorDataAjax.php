    <?php
        ...
        try{
            ...
            $lastLoadedRow = isset($headers['Last-Loaded-Row']) ? $headers['Last-Loaded-Row'] : 0;
            $entriesToLoad = isset($headers['Entries-To-Get']) ? $headers['Entries-To-Get'] : null;
            $startingRow = isset($headers['Starting-Element']) ? $headers['Starting-Element'] : null;
            // if(is_null($lastLoadedRow)){
            //     $lastLoadedRow = 0;
            // }
            if(!is_null($entriesToLoad)) {
                if(!is_null($startingRow)){
                    $sql = "SELECT `ID`, `Temperature(C)`, `Pressure(bar)`, `Humidity(%)`, `PM1`, `PM2.5`, `PM10`, `Timestamp` FROM Exterior WHERE ID >= " . $startingRow . " ORDER BY id DESC LIMIT ". $entriesToLoad;
                } else{
                    $sql = "SELECT `ID`, `Temperature(C)`, `Pressure(bar)`, `Humidity(%)`, `PM1`, `PM2.5`, `PM10`, `Timestamp` FROM Exterior WHERE ID > " . $lastLoadedRow . " ORDER BY id DESC LIMIT ". $entriesToLoad;
                }
            } else {       
